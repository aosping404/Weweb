...
const ModernNavbar = ({ onNavigateToGallery, onNavigateToOnScroll }) => {
  ...
  const navItems = [
    ...
    { name: "OnScroll", href: "#onscroll", subtitle: "ShapeMorph" }
  ];
                ...
                <li key={index} className="menu-nav-item">
                  <a
                    ...
                    onClick={(e) => {
                      closeMenu();
                      // 如果是相册链接，调用导航函数
                      if (item.name === '相册' && onNavigateToGallery) {
                        e.preventDefault();
                        onNavigateToGallery();
                      }
                      // 如果是 OnScroll 链接，调用导航函数
                      if (item.name === 'OnScroll' && onNavigateToOnScroll) {
                        e.preventDefault();
                        onNavigateToOnScroll();
                      }
                      // 其他链接让浏览器自然滚动到对应位置
                    }}
                  ...
                  >
                    <div className="menu-nav-link-content">
                      <div className="menu-nav-link-text-container">
                        ...
                        {item.subtitle && (
                          <span className="menu-nav-link-subtitle art-font">
                            {item.subtitle}
                          </span>
                        )}
                      </div>
