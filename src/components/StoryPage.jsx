...
const StoryPage = () => {
  ...
  return (
    <div className="min-h-screen w-screen bg-[#0a0a0f] text-[#5e9cdb] relative overflow-hidden">
      {/* 背景装饰 */}
      ...
      {/* 主内容 */}
      <div className="flex size-full flex-col items-center py-10 pb-24 pt-20 relative z-30">
        {/* 标题 */}
        <div className="text-center mb-16">
          <h1 className="text-6xl md:text-8xl font-bold bg-gradient-to-r from-[#5e9cdb] to-[#8bb3e8] bg-clip-text text-transparent mb-4">
            随机标语
          </h1>
          <p className="text-xl md:text-2xl text-[#8bb3e8] opacity-80">
            青春回忆 • 友谊
          </p>
        </div>
        ...
        <div className="flex items-center justify-center mt-10">
          <div className="w-full max-w-4xl h-[60vh] bg-transparent flex flex-col items-center justify-center">
            {/* 英文文案 */}
            <div className="mb-6 w-full">
              <AnimatedTitle
                key={`english-${currentQuoteIndex}`}
                title={displayQuote.english}
                containerClass="text-center text-[#5e9cdb] text-3xl md:text-4xl lg:text-5xl font-medium leading-relaxed"
              />
            </div>
            ...
            <div className="mb-4 w-full">
              <AnimatedTitle
                key={`chinese-${currentQuoteIndex}`}
                title={displayQuote.chinese}
                containerClass="text-center text-[#8bb3e8] text-xl md:text-2xl lg:text-3xl font-medium leading-relaxed"
              />
            </div>
            
            {/* 来源 */}
            <div className="w-full">
              <p className="text-center text-[#5e9cdb] text-base md:text-lg lg:text-xl opacity-80">
                {displayQuote.source}
              </p>
            </div>
          </div>
        ...
        <div className="mt-16 text-center">
          <p className="text-lg text-white/60">
            每 8 秒自动切换 • 共 {friendshipQuotes.length} 条标语
          </p>
        </div>
      </div>
    </div>
